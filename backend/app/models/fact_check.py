...
class FactCheckConfig(BaseModel):
    ...
    model_config_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("llm_configs.id", ondelete="SET NULL"), nullable=True)
...
class FactCheckRun(BaseModel):
    ...
    __table_args__ = (
        ...
        CheckConstraint("status IN ('PENDING', 'RUNNING', 'WAITING_CONFIRMATION', 'SUCCESS', 'FAILURE', 'CANCELLED')", name="ck_fact_check_status"),
        ...
        CheckConstraint("stage IN ('extract', 'check', 'complete')", name="ck_fact_check_stage"),
        CheckConstraint("depth IN ('standard', 'deep')", name="ck_fact_check_depth"),
    )

    search_provider: Mapped[str] = mapped_column(String(16), nullable=False, default="model", server_default="model")
    record_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("proofread_records.id", ondelete="SET NULL"), nullable=True)
    user_id: Mapped[int] = mapped_column(Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    title: Mapped[str] = mapped_column(String(200), nullable=False, default="", server_default=text("''"))
    source_kind: Mapped[str] = mapped_column(String(16), nullable=False, default="record", server_default="record")
    file_id: Mapped[str | None] = mapped_column(String(100), nullable=True)
    parent_run_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("fact_check_runs.id", ondelete="SET NULL"), nullable=True)
    confirm_claims: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, server_default=false())
    stage: Mapped[str] = mapped_column(String(16), nullable=False, default="extract", server_default="extract")
    depth: Mapped[str] = mapped_column(String(16), nullable=False, default="standard", server_default="standard")
    selected_claim_ids: Mapped[list[str] | None] = mapped_column(JSON(none_as_null=True), nullable=True)
    supplemental_urls: Mapped[list[str]] = mapped_column(JSON, nullable=False, default=list, server_default="[]")
    config_snapshot: Mapped[dict] = mapped_column(JSON, nullable=False, default=dict, server_default="{}")
    queued_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    execute_request_id: Mapped[str | None] = mapped_column(String(36), nullable=True)
    execute_request_hash: Mapped[str | None] = mapped_column(String(64), nullable=True)
...
class FactCheckReview(BaseModel):
    ...
    __table_args__ = (
        UniqueConstraint("run_id", "request_id", name="uq_fact_check_review_request"),
        CheckConstraint("decision IN ('agree', 'disagree', 'unresolved')", name="ck_fact_check_review_decision"),
    )

    run_id: Mapped[int] = mapped_column(Integer, ForeignKey("fact_check_runs.id", ondelete="CASCADE"), nullable=False, index=True)
    user_id: Mapped[int] = mapped_column(Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
