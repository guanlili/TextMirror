...
@router.get("", response_model=List[RoleResponse], summary='获取所有角色列表')
...
@router.post("", response_model=RoleResponse, status_code=status.HTTP_201_CREATED, summary='创建自定义角色')
...
@router.get("/permissions/tree", response_model=List[PermissionResponse], summary='获取权限树（树形结构）- 一次加载全部权限，Python 端构建树')
