...
async def _check_login_lock(employee_id: str, client_ip: str) -> None:
    ...
    try:
        ...
        logger.warning(f"登录锁定检查失败（不阻塞登录）: {e}")
...
async def _record_login_failure(employee_id: str, client_ip: str) -> None:
    ...
    try:
        redis = get_redis()
        key = f"textmirror:login_fail:{employee_id}:{client_ip}"
        pipe = redis.pipeline()
        pipe.incr(key)
        ...
        if int(count) >= settings.LOGIN_MAX_FAILURES:
            await redis.set(
                f"textmirror:login_lock:{employee_id}:{client_ip}",
                "1",
                ex=settings.LOGIN_LOCK_SECONDS,
            )
            logger.warning(f"登录失败次数过多，已锁定: {employee_id} IP={client_ip}")
    except Exception as e:
        logger.warning(f"登录失败计数失败（不阻塞登录）: {e}")
...
async def _clear_login_failures(employee_id: str, client_ip: str) -> None:
    ...
    try:
        redis = get_redis()
        await redis.delete(
            f"textmirror:login_fail:{employee_id}:{client_ip}",
            f"textmirror:login_lock:{employee_id}:{client_ip}",
        )
    except Exception as e:
        logger.warning(f"清除登录失败计数失败: {e}")
...
async def login(request: LoginRequest, http_request: Request, db: AsyncSession = Depends(get_db)):
    ...
    连续失败 5 次将临时锁定 15 分钟（等保基线要求）
    """
    client_ip = get_client_ip(http_request)
    user_agent = http_request.headers.get("User-Agent", "")

    await _check_login_lock(request.employee_id, client_ip)
    ...
    if user is None:
        await _record_login_failure(request.employee_id, client_ip)
    ...
    # 登录成功，清除失败计数
    await _clear_login_failures(request.employee_id, client_ip)
...
@router.put("/profile", response_model=ProfileUpdateResponse, summary='修改个人信息')
