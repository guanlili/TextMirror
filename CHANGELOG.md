...
- 精简中英文 README、使用 / 运维 / 设计文档，纠正运行环境、默认 HTTP、额度及文件导出边界，补充新功能入口和验证指引。

以上功能对应 [PR #84](https://github.com/guanlili/TextMirror/pull/84)。包含审阅、质量反馈、事实核查配置及任务、搜索服务选择、用量账本五个数据库迁移，升级前需备份并同步更新 API、worker 和前端。事实核查仍需在实际部署中启用并完成真实联网验收。
