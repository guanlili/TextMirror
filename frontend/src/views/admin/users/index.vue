    ...
    <el-dialog v-model="showCreateDialog" :title="editingUser ? '编辑用户' : '创建用户'" width="500px" @close="resetForm">
      <el-form :model="form" label-width="80px">
        ...
        <el-form-item label="日配额">
          <el-input-number v-model="form.daily_quota" :min="1" :max="100000" clearable placeholder="留空不限" />
          <div style="font-size: 12px; color: #999; margin-top: 4px;">留空表示不限次数</div>
        </el-form-item>
...
function editUser(row: AdminUserItem) {
  ...
  form.value = { employee_id: row.employee_id, username: row.username, password: '', role_id: row.role_id || 0, department: row.department || '', phone: row.phone || '', daily_quota: row.daily_quota ?? null, remark: row.remark || '' }
  showCreateDialog.value = true
}

function resetForm() {
  editingUser.value = null
  form.value = { employee_id: '', username: '', password: '', role_id: 0, department: '', phone: '', daily_quota: null, remark: '' }
}
