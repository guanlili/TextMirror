<template>
  <section
    ...
    aria-label="协作审校进度"
    data-testid="collaboration-progress"
  >
    <header>
      <strong>协作审校</strong>
      ...
      <span
        v-if="report?.model_name"
        class="muted"
      >共享模型：{{ report.model_name }}</span>
    </header>
    <p class="pipeline">
      规则检查 → 语言与一致性并行审校 → 一轮复核（最多 20 条）→ 人工决定
    </p>
    <p class="muted">
      规则检查不是 AI Agent。一次流程扣一次应用额度；最多 3 次模型调用，另可能有服务商传输重试，实际供应商费用按 Token 计。不会自动联网、无限辩论或自动采纳修改。
    </p>
    <p
      v-if="message"
      role="status"
    ...
    >
      <article
        v-for="role in report.roles"
        :key="role.id"
        :data-role="role.id"
        :data-status="role.status"
      >
        <div class="role-heading">
          <strong>{{ role.id === 'rules' ? '规则检查' : role.name }}</strong><el-tag
            size="small"
            :type="collaborationStatusTypes[role.status]"
          >
            {{ collaborationStatusLabels[role.status] }}
          </el-tag>
        </div>
        <p>{{ role.message }}</p>
        <p class="muted">
          发现 {{ role.issue_count }} 条 · 已报告耗时 {{ (role.elapsed_ms / 1000).toFixed(1) }} 秒
        </p>
        <p class="muted">
          {{ usageLabel(role.usage) }}
        </p>
      </article>
    </div>
    <p
      ...
      class="muted"
    >
      已发现 {{ report.findings.length }} 条 · 已复核 {{ report.reviewed_count }} / 最多 {{ Math.min(20, report.review_limit) }} 条。未复核不等于已确认；争议建议请人工把关。
    </p>
    <p
      v-if="report?.status === 'partial' || failed"
      class="warning"
      role="alert"
    >
      流程未完整完成；保留此前发现，仅供参考。不能通过普通补查宣称协作完成。可手动重新运行完整流程（另计额度与费用）。
    </p>
    <p
      v-if="cancelRequested && !terminal"
      class="warning"
      role="status"
    >
      取消请求已发送，等待服务端确认；正在执行的请求可能先完成并产生费用。
    </p>
    <div
      v-if="error"
      class="warning"
      role="alert"
    >
      状态连接中断：{{ error }}。重连只读取原任务，不会重新提交。
    </div>
    <div class="actions">
      <el-button
        v-if="taskId && !monitoring"
        size="small"
      ...
      <el-button
        v-if="(report?.status === 'partial' || failed) && (terminal || !taskId)"
        size="small"
        type="warning"
        plain
        @click="$emit('rerun')"
    ...
    <details
      v-if="report?.findings.length && viewOnly"
      class="findings"
    >
      ...
      <ol>
        <li
          ...
          :key="`${finding.start ?? ''}-${finding.end ?? ''}-${finding.type}-${index}`"
        >
          <p>{{ finding.original }} → {{ finding.suggestion || '需人工核对' }}</p>
          <p class="muted">
            {{ collaborationProvenance(finding, report) }}
          </p>
          <p
