...
describe('fact-check API contract', () => {
  ...
  it('Tavily 保存裁剪字段、携带 provider，空密钥省略、默认根路径', async () => {
    ...
    await saveFactCheckSettingsApi({ enabled: false, provider: 'tavily', max_claims: 10, api_key: ' ', sources: [source] }, { signal })
    expect(request.put).toHaveBeenLastCalledWith('/admin/fact-check/settings', {
      enabled: false, provider: 'tavily', max_claims: 10, sources: [{ id: 'source-1', name: '来源一', domain: 'example.org', path_prefix: '/', is_enabled: false }],
    }, { signal, headers: { 'X-Silent-Error': 'true' } })
    expect(source.name).toBe('  来源一 ')
    await saveFactCheckSettingsApi({ enabled: true, provider: 'tavily', max_claims: 10, api_key: ' new-secret ', sources: [] })
    expect(request.put).toHaveBeenLastCalledWith('/admin/fact-check/settings', { enabled: true, provider: 'tavily', max_claims: 10, api_key: 'new-secret', sources: [] }, expect.anything())
  })
  it.each([true, false])('原生模式保存 enabled=%s 始终不发送密钥或只读模型信息', async enabled => {
    const payload = {
      enabled, provider: 'model', max_claims: 10, sources: [], api_key: 'unsent-tavily-secret',
    ...
    expect(request.put).toHaveBeenCalledExactlyOnceWith('/admin/fact-check/settings', {
      enabled, provider: 'model', max_claims: 10, sources: [],
  ...
  it.each(['model', 'tavily'] as const)('%s 响应保留当前模型描述与已保存任务 provider，不触发探测或启动', async provider => {
    ...
    const run: FactCheckRun = {
      id: 21, record_id: 7, provider, mode: 'web', status: 'SUCCESS', progress: 100, message: '', error_code: null,
